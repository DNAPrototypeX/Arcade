...
def fs_main():
    import pygame
    from fs_ship import Ship
    from fs_buildings import Buildings
    import sys
    ...
    f = open("fs_highscore.txt", "r")
    high_score = f.read()
    high_score_text = font.render("HighScore: " + str(high_score), True, RED)
    f.close()
    background = pygame.image.load('fs_background.jpg').convert()

    background_x = 0
    pygame.mixer.music.load('fs_music.wav')
